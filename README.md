...
デモはStreamlit Community Cloudで確認できます。(一部文字化けがございます)
...
1. YAMLファイルをテキストエディタで開きます（UTF-8で作成すること）
3. 複雑さ、戦略的価値、相互作用値を実際のゲーム体験に基づいて調整します
4. 変更を保存し、アプリを再起動します
...
1. 埋め込みデータ生成: ゲームの情報（名前、説明、カテゴリ、メカニクスなど）をAIモデルによって埋め込みモデル（多次元ベクトル）に変換
...
類似性検索を使用するには、事前にエンベディングデータファイル（game_embeddings.pkl）を用意する必要があります。配布しているこのファイルを使用するのであれば、自分で埋め込みデータを作成する必要はありません。
...
generate_embedding_model.pyを使用して、YAMLファイルから保存されたゲームデータの埋め込みモデルを生成できます：
...
- 自分で埋め込みデータを生成する場合は、Voyage AIのユーザー登録が必要です。また支払い設定前はパフォーマンス制限があります
